...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function () {
    navigator.serviceWorker.register('/sw.js').then(function (reg) {
      // Force an update check now rather than waiting on the browser's own
      // (sometimes long-delayed, especially on iOS Safari) heuristic for
      // re-checking sw.js — it's served no-cache, so this always hits
      // network. Re-check on every foreground too, since a backgrounded
      // mobile tab can sit resumed-but-unrefreshed for a long time without
      // a fresh navigation of its own.
      reg.update();
    ...
    // Once a new service worker actually activates and takes control (see
    // skipWaiting + clients.claim in sw.js), reload so this tab picks up the
    // fresh bundle instead of continuing to run the old one it already has
    // in memory — without this, a visitor who already has the app open
    // stays stuck on stale code until they manually reload. Guarded to only
    // fire for a return visit that already had a controller, so a brand new
    // install doesn't reload itself right after first load.
