...
`sim1not2p1`, `aya`). Shipped zipped and loaded on first run. `QURAN_WORDS = 77881` is the
total word count (the DB's last `_id`) and the wrap-around boundary; `SURA_IDX` in
`www/src/models/constants.ts` must stay in sync with the shipped `q.json` tokenization.
